...
from .models import Comment, ArticlePost, ArticleColumn
...
def article_titles(request,username=None):
    if username:
        ...
        articles_title = ArticlePost.objects.filter(author=user1)
        try:
    ...
    try:
        ...
        articles = current_page.object_list
    if username:
        return render(request, 'article/list/author_articles.html', {'articles':articles,'page':current_page,'userinfo':userinfo,'user1':user1})
...
def article_detail(request, id, slug):
    ...
    # similar_articles = ArticlePost.objects.filter(article_tags__in=article_tags_ids).exclude(id=article.id)
